...
int exec_redir(char *str) { //Executes command with redirection (<, >, >>).
	//Parses command's string.
	char** args = parse(str);
	char *parameters[20];
	...
	//Checks if there is one or two instances of input, or output redirection (including >>).
	int input_count = 0;
	int output_count = 0;
	int l_count = 0;
	if (args[1][0] == '>') {
	...
	//CHecks which kind of redirection will be used, then executes command.
	if ((input_count == output_count) == 1) { //Execution with input and output redirection (< and >).
		pid_t pid = fork();
		if (pid == 0) {
		...
		return 0;
	 } else if ((input_count == 1) && (output_count == 2)) { //Execution with input and output redirection (< and >>).
		pid_t pid = fork();	
		if (pid == 0) {
			int fd_in = open(args[i+1], O_RDONLY);
	...
	} else if ((input_count == 0) && (output_count == 2)) { //Execution with only output redirection (>>).
		pid_t pid = fork();
		if (pid == 0) {
			int fd = open(args[i+1], O_WRONLY|O_CREAT|O_APPEND,S_IRWXU|S_IRWXG|S_IRWXO); //Open to append to file.
			dup2(fd, 1);
			if (check_builtins(args, str) == 0) {	
				exit(0);
			}
	...
	} else if ((input_count == 1) && (output_count == 0)) { //Execution with only input redirection. (<).
		pid_t pid= fork();
		if (pid == 0) {
         ...
         } else if ((input_count == 0) && (output_count == 1)) { //Execution with only output redirection. (>).
		pid_t pid = fork();
		if (pid == 0) {
...
int exec_pipe(char* str) { //Executes command with pipe redirection (|).
	//Parses commands' strings.
	char** args = parse(str);
	char *path1 = args[0];
	char* parameters1[20];
	char* parameters2[20];
...
int check_builtins(char **args, char* str) { //Checks for builtin commands, then executes builtin command if called.
	...
	if (strcmp(args[0], "cd") == 0) { //cd - Changes directory.
		...
		if (args[1] == NULL) {
			...
			if ((dp = opendir(path)) == NULL) { //Opens directory.
				printf("opendir %s error", path);
			}
			while ((list = readdir(dp))) {
				printf("%s\n", list -> d_name); //Reads directory.
			}
			closedir(dp);
		}
		return 0;
		...
		FILE *help;
		if ((help = fopen("help", "r")) == NULL) { //Opens the file named "help" and reads it.
			printf("error opening help file.\n");
		}
		char str_help[1024];
		while (fgets(str_help, 1024, help)) {
			printf("%s", str_help);
		}
		return 0;	
