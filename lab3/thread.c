...
void* reporter() {
	...
	fprintf(fp, "Reporter Signals, TIME(Sec), TIME(Microsec), TIME STAMP(Sec + Microsec), SIGUSR1 Count, AVG TIME 1, SIGUSR2 Count, AVG TIME 2\n");
	...
	while (1) {
		...
		//	printf("sig1_rec = %d, sig2_rec = %d\n", sig1_rec, sig2_rec);	
			...
			//	printf("avg2 = %d microseconds\n", avg2);
				fprintf(fp, "%d, %d, %d, %d, %d, %d, %d, %d\n", reporter_sigs,(int) tv1.tv_sec,(int) tv1.tv_usec, ((unsigned int)tv1.tv_sec * 1000000 + (unsigned int) tv1.tv_usec), sig1fortime, avg1, sig2fortime, avg2);				
